...
export default function GameSection() {
  ...
  const [miningProbes, setMiningProbes] = useState(0);
  const [isLoaded, setIsLoaded] = useState(false);
  const [shipUnlocked, setShipUnlocked] = useState(false); // ESTADO DE LA NAVE

  const PROBE_COST = 50;
  const SHIP_THRESHOLD = 20000; // META DE 20K

  // Cargar progreso al montar el componente
  useEffect(() => {
    const savedCredits = localStorage.getItem("odyssey_credits");
    const savedProbes = localStorage.getItem("odyssey_probes");
    const savedShip = localStorage.getItem("odyssey_ship_unlocked");
    
    if (savedCredits) setCredits(parseFloat(savedCredits));
    if (savedProbes) setMiningProbes(parseInt(savedProbes));
    if (savedShip === "true") setShipUnlocked(true);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("odyssey_credits", credits.toString());
    localStorage.setItem("odyssey_probes", miningProbes.toString());
    localStorage.setItem("odyssey_ship_unlocked", shipUnlocked.toString());
  }, [credits, miningProbes, isLoaded, shipUnlocked]);
  ...
  const handlePlanetClick = () => {
    ...
    // LÓGICA DE DESBLOQUEO DE NAVE
    if (newCredits >= SHIP_THRESHOLD && !shipUnlocked) {
      setShipUnlocked(true);
      alert("🚀 ¡BRUTAL! Has desbloqueado la 'Odyssey Explorer'. La galaxia es tuya.");
    }
  };
  ...
  const resetProgress = () => {
    if (confirm("Alerta de la Flota: ¿Estás seguro? Se perderán créditos, sondas y naves.")) {
      setCredits(0);
      setMiningProbes(0);
  ...
  // Bucle principal del juego "Idle"
  useEffect(() => {
    if (miningProbes === 0) return;
    const interval = setInterval(() => {
      setCredits((prev) => {
        const nextCredits = prev + miningProbes;
        // También chequear 20k con las sondas automáticas
        if (nextCredits >= SHIP_THRESHOLD && !shipUnlocked) {
          setShipUnlocked(true);
        }
        return nextCredits;
      });
      ...
      <div className="w-full max-w-5xl bg-card rounded-2xl border border-accent/20 flex flex-col md:flex-row relative overflow-hidden shadow-[0_0_60px_rgba(0,240,255,0.05)] min-h-[500px]">
        ...
        <div className="flex-1 flex flex-col justify-center items-center p-8 relative z-10 border-b md:border-b-0 md:border-r border-white/5">
          ...
          <div className="flex justify-between items-center w-full max-w-sm mb-2">
             ...
             <h2 className="text-xl md:text-2xl text-accent/80 uppercase tracking-[0.2em] text-center">Créditos Estelares</h2>
             <button onClick={resetProgress} className="w-6 h-6 text-foreground/30 hover:text-red-500 transition-colors">
               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                 <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
               </svg>
             </button>
          ...
          <button onClick={handlePlanetClick} className="w-56 h-56 md:w-64 md:h-64 rounded-full bg-gradient-to-br from-cyan-300 via-accent to-blue-900 shadow-[0_0_50px_rgba(0,240,255,0.5)] active:scale-90 transition-transform duration-100 flex items-center justify-center group border-4 border-accent/30">
            <div className="w-[85%] h-[85%] rounded-full border border-white/20 bg-gradient-to-tr from-transparent to-white/20 group-active:opacity-30 transition-opacity"></div>
          </button>

          {/* MENSAJE DE NAVE DESBLOQUEADA */}
          {shipUnlocked ? (
            <div className="mt-8 p-4 border-2 border-yellow-500 bg-yellow-500/10 rounded-lg animate-pulse">
              <p className="text-yellow-400 font-bold text-center tracking-widest uppercase text-sm">
                🛸 EXPLORADOR GALÁCTICO ACTIVO
              </p>
            </div>
          ) : (
            <p className="mt-8 text-xs text-foreground/40 uppercase tracking-[0.3em] animate-pulse select-none">
              Meta: 20,000 para la Odyssey Explorer
            </p>
          )}

          <div className="absolute bottom-4 left-4 flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${isLoaded ? 'bg-green-500 shadow-[0_0_5px_#22c55e]' : 'bg-yellow-500'}`}></div>
            <span className="text-[10px] text-foreground/40 uppercase tracking-widest">{isLoaded ? 'Save: OK' : 'Syncing...'}</span>
          </div>
        </div>

        {/* TIENDA */}
        <div className="w-full md:w-80 bg-black/40 p-6 flex flex-col z-10 backdrop-blur-md">
          <h3 className="text-lg md:text-xl text-white mb-6 uppercase tracking-[0.1em] border-b border-white/10 pb-4">Flota</h3>
          <div className="bg-card/70 p-5 border border-white/5 rounded-xl flex flex-col group">
            <div className="flex justify-between items-start mb-3">
              <span className="font-bold text-md">Sonda de Minería</span>
              <span className="text-xs bg-accent/10 border border-accent/30 text-accent px-2 py-1 rounded-sm">Lv. {miningProbes}</span>
            </div>
            <p className="text-xs text-foreground/50 mb-5 leading-relaxed">Extrae <span className="text-accent">+1/seg.</span></p>
            <button onClick={buyProbe} disabled={credits < PROBE_COST} className={`w-full py-3 rounded-md text-sm font-bold uppercase transition-all ${credits >= PROBE_COST ? "bg-accent/10 text-accent border border-accent hover:bg-accent hover:text-background" : "bg-white/5 text-foreground/30 cursor-not-allowed"}`}>
              Comprar ({PROBE_COST} C)
            </button>
          </div>
          <div className="mt-auto pt-8 text-[10px] text-accent/30 tracking-[0.2em] text-center uppercase">Aether Systems v1.0</div>
        </div>
